...
# Training loop HPs
EPOCHS = 30
TRAJECTORY_BATCH_SIZE = 8 # Number of initial sequences to generate trajectories from in one go
...
# --- GAE Calculation - Unchanged structurally, but rewards input will be sparse ---
@tf.function
def calculate_gae(rewards, values, dones, gamma=GAMMA, lambda_gae=LAMBDA_GAE):
    ...
    future_advantage = tf.constant(0.0, dtype=tf.float32)
    last_value = values[:,-1] # V(s_T) estimated by critic
    rewards = tf.cast(rewards, dtype=tf.float32)
    values = tf.cast(values, dtype=tf.float32)

    for t in tf.range(tf.shape(rewards)[1] - 1, -1, -1):
        # next_value = values[t+1] # This was slightly wrong index-wise before for terminal step
        # Correct logic: V(s_{t+1}) is needed. If t is the last step (T-1), next_value should be 0 if done[T-1] is True.
        # Since we assume fixed length and set done[-1]=True, the last V(s_T) estimate is in last_value.
        next_value_bootstrap = values[:,t+1] if t < tf.shape(rewards)[1] - 1 else last_value
        mask = 1.0 - tf.cast(dones[:,t], tf.float32) # 0 if done, 1 otherwise
    ...
    returns = advantages + values[:,:-1] # Target for V(s_t) is GAE_t + V(s_t)
...
# --- Data Collection Function (Modified for Sparse Rewards) ---
def collect_trajectories_sparse(initial_states_batch, actor, critic, num_steps, sigma):
    batch_states = []
    batch_actions = []
    batch_log_probs = []
    batch_rewards = [] # Will be mostly zeros
    batch_values = []
    batch_dones = []
    batch_full_sampled_sequences = []
    batch_seq_pretrained = []
    final_rewards = []

    for initial_state in initial_states_batch:
        states, actions, log_probs, values, dones = [], [], [], [], []
        # Initialize rewards list with zeros
        rewards = [0.0] * num_steps
        current_sequence = tf.expand_dims(initial_state, 0)
        current_generated = []

        seq_pre = generate_sequence_sampling(pretrained_model, current_sequence, num_steps, 0.0) # Deterministic
        batch_seq_pretrained.append(seq_pre)
        ...
        for t in range(num_steps):
            ...
            # Clip action here *after* sampling, before storing/using
            action_t = tf.clip_by_value(action_t, 0.0, 1.5)
            log_prob_t = dist_t.log_prob(action_t) # Log prob of the *clipped* action? Better to use unclipped. Recalc if needed.
            ...
            dones.append(False) # Done is False until the very end

            # Prepare next state
            current_generated.append(tf.identity(action_t)) # Store clipped action
            next_step_reshaped = tf.reshape(action_t, [1, 1, N_FEATURES])
            current_sequence = tf.concat([current_sequence[:, 1:, :], next_step_reshaped], axis=1)

        # --- After the loop: Calculate Final Reward ---
        # current_generated.append(tf.constant([[END_TOKEN]], dtype=tf.float32)) # Append END_TOKEN
        full_sampled_seq = tf.concat(current_generated, axis=0)
        final_reward = calculate_final_reward(full_sampled_seq, seq_pre)
        rewards[-1] = final_reward.numpy() # Assign final reward to the last step
        # ---------------------------------------------
        ...
        dones[-1] = True # Mark end for GAE

        batch_states.append(states)
        batch_actions.append(actions)
        batch_log_probs.append(log_probs)
        batch_rewards.append(rewards) # Now contains sparse rewards
        batch_values.append(values)
        batch_dones.append(dones)
        batch_full_sampled_sequences.append(full_sampled_seq)
    ...
    batch_log_probs = tf.convert_to_tensor(np.array(batch_log_probs), dtype=tf.float32)
    batch_rewards = tf.convert_to_tensor(np.array(batch_rewards), dtype=tf.float32)
    batch_values = tf.convert_to_tensor(np.array(batch_values), dtype=tf.float32)
    batch_dones = tf.convert_to_tensor(np.array(batch_dones), dtype=tf.bool)
    try:
        batch_full_sampled_sequences = tf.stack(batch_full_sampled_sequences)
    except tf.errors.InvalidArgumentError as e:
         print(f"Error stacking full sequences: {e}")
         print("Shapes of sequences collected:")
         #for i, seq in enumerate(batch_full_sampled_sequences):
         #    print(f" Seq {i}: {tf.shape(seq)}") # Debug shapes if needed
         # Handle error or return indicator
         return None # Indicate failure

    # Calculate advantages and returns using sparse rewards
    batch_advantages, batch_returns = calculate_gae(batch_rewards, batch_values, batch_dones)
...
for epoch in range(EPOCHS):
    ...
    if full_sequences_sampled.shape[0] != num_traj or len(seq_pretrained_collected) != num_traj:
         print(f"  Warning: Dimension mismatch. Skipping PPO updates for epoch {epoch+1}.")
         continue
